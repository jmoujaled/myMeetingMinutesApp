-- Advanced search and filtering functions for transcription history
-- This migration implements full-text search, date filtering, and relevance scoring
...
-- Function to get user transcription history with advanced search and filtering
...
-- Function to get total count for pagination (optimized version)
...
-- Function for advanced search with highlighting and snippets
...
-- Function to get search suggestions based on user's content
...
-- Function to get search analytics for user's content
...
BEGIN
  ...
  WITH analytics_data AS (
    SELECT 
      ...
      COUNT(CASE WHEN m.transcript_text IS NOT NULL THEN 1 END) as meetings_with_transcripts_count,
      AVG(LENGTH(COALESCE(m.transcript_text, '')))::INTEGER as avg_length,
  ...
  SELECT 
    ...
    (
      ...
      FROM (
        ...
        FROM UNNEST(ad.all_topics) AS topic
        GROUP BY topic
